...
export class BlockChainService {
  ...
  private METADATA_PROGRAM_ID = new PublicKey(
  ...
  async getTransctionsSignatures(walletAddress: string): Promise<
    {
      tokenName: string;
  ...
  async getTransactSignatureInfo(signatures: ConfirmedSignatureInfo[]): Promise<
    {
      tokenName: string;
      formattedDate: string;
      amountTransferred: number;
      ...
      const transactions = await Promise.all(
        signatures.map(async (tx) => {
          ...
          const formattedDate = transaction.blockTime
            ? formatDate(new Date(transaction.blockTime * 1000))
            : 'Unknown Date';
          // console.log(transaction.meta?.postTokenBalances)
          const mintAddress =
            transaction.meta?.postTokenBalances?.[0]?.mint || 'Unknown Mint';
          ...
          if (transaction.meta?.logMessages) {
            ...
            const isTransfer = logs.some((log: string) =>
              /Instruction: (Transfer|TransferChecked|TransferToken)/i.test(
                log,
              ),
            );
            ...
            if (isTransfer) {
              ...
              if (preBalances.length > 0 && postBalances.length > 0) {
                ...
                const hasNegativeChange = balanceChanges.some(
                  (change) => change.change < 0,
                );
  ...
  async getTokensOnAccount(walletAddress: string): Promise<any> {
    ...
    const address = new PublicKey(walletAddress);
    const tokenAccounts = await this.solana.getParsedTokenAccountsByOwner(
      address,
      {
